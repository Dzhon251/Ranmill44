import {ChangeDetectorRef, Component, OnDestroy, OnInit, Renderer2, signal, ViewChild} from '@angular/core';
import {FooterComponent} from '../../component/footer/footer.component';
import {RightmenuComponent} from '../../component/rightmenu/rightmenu.component';
...
export default class TemplateComponent implements OnDestroy,OnInit {


  readonly activeSpinner = signal<boolean>(false);
  ...
  ngOnInit(): void {
    ...
    this.activeSpinner.set(false)
  }
  constructor(
    private menuService: MenuService,
    public layoutService: TemplateService,
      ...
      this.layoutService.menuProfileOpen$.subscribe(() => {
        ...
        if (!this.menuProfileOutsideClickListener) {
          this.menuProfileOutsideClickListener = this.renderer.listen(
            ...
            (event: { target: any; }) => {
              const isOutsideClicked = !(
                this.appSidebar.usuarioMenu?.el?.nativeElement.isSameNode(
                  event.target
                ) ||
                this.appSidebar.usuarioMenu?.el?.nativeElement.contains(
                  event.target
                )
              );
              if (isOutsideClicked) {
